...
void PlayerUpdate(Player* player)
{
	...
	BulletUpdate(player->bdata, player->ModelHandle, player->position);
		...
		//{
		//	// フラグが立っていたら処理
		//	if (player->bullet[i]->Flag)
		//	{
		//		// 消滅時間を１フレームで１増やす
		//		player->bullet[i]->DeleteCount++;
		//		// 時間が一定を超えたら消滅させる
		//		if (++player->bullet[i]->DeleteCount > SHOTDELETETIME)
		//		{
		//			// フラグを折る
		//			player->bullet[i]->Flag = false;
		//		}
		//		// ポジションアタッチ
		//		MV1SetPosition(player->bullet[i]->ModelHandle, player->bullet[i]->position);
		//	}
		//}
